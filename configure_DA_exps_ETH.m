function [Ol, Y, R, Xa, da] = configure_DA_exps_ETH(model, x0, Active, Recovered, Deaths, Vaccinated, my_config_ETH)

% Ensemble Size
da.Ne    = my_config_ETH.Ne;
da.denom = (model.Nx - 1)*da.Ne;

da.w     = my_config_ETH.w;              % Additive inflation
da.clamp = my_config_ETH.clamp;          % Clamping state value

da.anamorph = my_config_ETH.anamorph;    % State Transformation
da.inflate  = my_config_ETH.inflate;     % Multiplicative inflation
da.filter   = my_config_ETH.filter;      % Filter kind [EAKF, EnKF, RHF] 
...
da.data_type = my_config_ETH.data_type; 
...
[~, Ol, Y, R, da] = observer_ETH(model.Nx, da, obs_ervar, Active, Recovered, Deaths, Vaccinated);
